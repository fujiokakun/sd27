...
<?php
	...
	$args_user = $_POST['user'];
	$args_user_address = $_POST['user_address'];

	$user_dummy = new User($args_user);
	$user_address = new User_address($args_user_address);


	$user_address->user_id = $_POST['user']['user_id'];

	//	$product->merge_attributed($args);
	$result = $user_dummy->update();
	$user_address->update();
					...
					<div class="col-12 col-xl-6">
						...
						<table class="table table-bordered table-striped bg-light text-center">
							...
							<?php // Populate product table:
								...
								$users = User::find_by_id($_POST['user']['user_id']);
								$users_address = User_address::find_by_sql("SELECT * FROM user_address WHERE user_id = {$_POST['user']['user_id']}");
